...
go run ./cmd/client --master "$MASTER" mkdir "$PREFIX"
go run ./cmd/client --master "$MASTER" create "${PREFIX}/ok.txt"
echo -n 'grpc-smoke-ok' >"$TMP"
go run ./cmd/client --master "$MASTER" write "${PREFIX}/ok.txt" "$TMP"
go run ./cmd/client --master "$MASTER" read "${PREFIX}/ok.txt" "${TMP}.out"
got="$(cat "${TMP}.out")"
