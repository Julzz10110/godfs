#!/usr/bin/env bash
# Wait until a Raft leader is visible (metrics and/or masters list); dump compose logs on failure.
set -euo pipefail
...
metrics_is_leader() {
  local port="$1"
  local body
  body=$(curl -sf "http://127.0.0.1:${port}/metrics" 2>/dev/null) || return 1
  echo "$body" | grep -qE '^godfs_raft_is_leader (1|1\.0+)( |$)'
}

grpc_has_leader() {
  local out leader
  out=$(go run ./cmd/client --master "$MASTER_GRPC" masters list 2>/dev/null) || return 1
  leader=$(echo "$out" | sed -n 's/^leader_node_id=//p' | tr -d '\r')
  [[ -n "$leader" ]]
}
